...
- Seções de dor e de fechamento "encharcadas" em âmbar — os momentos de inversão
...
### Primary
- **Amber Ledger** (`#E8952A`): O único acento de ação. Usado em CTAs primários, marcadores de lista, bordas de estado ativo e no fundo das seções de dor e de fechamento. Sua raridade nas superfícies escuras é intencional.
...
- **Ink Dark** (`#141210`): Superfície secundária — seção gestor e card de preço.
...
- **Stone Light** (`#9E9990`): Texto de suporte.
...
**Ritmo de seções:**
- `padding: 80px` — how, gestor, price
- `padding: 90px/80px` — pain (topo maior por causa da curva escura)
- `padding: 64px` — features
- Fechamento: sem padding vertical no container; o texto tem `80px` e a imagem ancora na borda inferior

**Breakpoints:** 800px (hero e gestor colapsam), 640px (steps e close colapsam), 480px (CTA vira full-width).
...
A exceção são figuras recortadas sobre fundo escuro:

### Shadow Vocabulary
- **Image Drop Shadow** (`drop-shadow(0 24px 64px rgba(0,0,0,0.55))`): Sombra de figura — usada na imagem das telas do gestor (telas-gestor).

### Named Rules
**A Regra do Plano-por-Padrão.** Superfícies são planas em repouso. Sombras existem apenas em figuras recortadas sobre fundo (imagens PNG/WebP).

## Shapes

Linguagem de formas consistentemente arredondada — sem ângulos retos em componentes interativos.

- **Pílula (99px):** Todos os botões. Forma default de interação.
- **Card (20px):** Card de preço. Arredondamento generoso mas não circular.
- **Base (10px):** containers menores.
- **Círculo (50%):** Marcador numérico dos passos.
- **Imagem do hero (16px):** `mockuphero.png`, levemente arredondada.
...
### Imagem do hero
- Imagem estática `mockuphero.png` (520×520), `max-width: 520px`, border-radius 16px.
- Entrada: `blurImg` 0.9s (desfoque → nítido). Flutuação: `phonefloat` 5s ease-in-out infinite, translateY 0 → -5px.
...
- Logo: imagem PNG `/logoagen+.png`, height 120px (a arte tem respiro interno).
- Sem links de texto: apenas logo + CTA `btn-amber` pequeno ("Criar minha agenda").
