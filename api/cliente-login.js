module.exports = async function handler(req, res) {
  ...
  if (req.method === 'PATCH') {
    ...
    if (!agendamento_id || !nome || !telefone || !acao) {
      return res.status(400).json({ error: 'Campos obrigatórios: agendamento_id, nome, telefone, ação' });
    }
    ...
    if (!Array.isArray(rows) || !rows.length) {
      return res.status(403).json({ error: 'Agendamento não encontrado ou não pertence a este cliente.' });
    }
  ...
  if (!Array.isArray(agendamentos) || agendamentos.length === 0) {
    return res.status(404).json({
      error: 'Cliente não encontrado. Verifique seu nome e WhatsApp conforme foram cadastrados.'
    });
  }
